...
export const api = {
  ...
  manualTrades: (query?: RequestQuery) => getJson<Record<string, unknown>>("/api/manual-trades", query),
  createManualTrade: (payload: unknown) => postJson<Record<string, unknown>>("/api/manual-trades", payload),
  updateManualTrade: (tradeId: string, payload: unknown) => putJson<Record<string, unknown>>(`/api/manual-trades/${encodeURIComponent(tradeId)}`, payload),
  deleteManualTrade: (tradeId: string) => deleteJson<Record<string, unknown>>(`/api/manual-trades/${encodeURIComponent(tradeId)}`),
